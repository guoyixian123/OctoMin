...
struct CLI {
    ...
    func compress(args: [String]) async {
        var source: String?
        var output: String?
        var algorithm: OMZCompressionAlgorithm = .zstd
        var level: OMZCompressionLevel = .default
        var password: String?
        ...
        if let o = output {
            ...
            outputURL = sourceURL.appendingPathExtension("omz")
        }
        
        let options = OMZCompressionOptions(
            algorithm: algorithm,
            level: level,
            blockSize: 1024 * 1024,
