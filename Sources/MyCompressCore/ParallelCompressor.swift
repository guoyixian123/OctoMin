...
/// 压缩进度报告
public struct MCZCompressionProgress: Sendable {
    /// 已处理的文件数
...
/// 压缩选项
public struct MCZCompressionOptions: Sendable {
    /// 压缩算法
    public var algorithm: MCZCompressionAlgorithm
    /// 压缩级别
    public var level: MCZCompressionLevel
    /// 块大小 (用于并行分块压缩)
    ...
    public init(
        ...
        level: MCZCompressionLevel = .default,
...
public actor ParallelCompressor {
    /// 压缩选项
    private let options: MCZCompressionOptions

    /// 进度回调 (在 actor 上调用, 线程安全)
    public typealias ProgressHandler = @Sendable (MCZCompressionProgress) -> Void
    ...
    public init(
        options: MCZCompressionOptions = MCZCompressionOptions(),
    ...
    public func compressFiles(_ fileURLs: [URL], sourceBase: URL, destination: URL) async throws {
                ...
                group.addTask { [options] in
                    ...
                    if info.isDirectory {
                        ...
                        return (idx, CompressedFile(
                            info: info,
                            data: emptyData,
                            crc32: MCZCRC32.compute(emptyData),
                            blockSizes: [],
                            uncompressedSize: 0
                        ))
                    }
                    ...
                    do {
                        ...
                        throw MCZFormatError.readError("无法读取文件 \(info.url.path): \(error.localizedDescription)")
                    }

                    let fileCrc = MCZCRC32.compute(fileData)
        ...
        // 报告完成
        progressHandler?(MCZCompressionProgress(
    ...
    private func writeArchive(
        ...
        let headerPlaceholder = Data(count: MCZFileHeader.size)
        ...
        // 回填文件头
        let flags: MCZFlags = crypto != nil ? [.encrypted] : []
        let header = MCZFileHeader(
            flags: flags,
    ...
    private func reportProgress(totalFiles: Int, totalBytes: Int64, currentFile: String?) {
        let progress = MCZCompressionProgress(
    ...
    public static func collectFiles(from directoryURL: URL) throws -> [URL] {
        ...
        guard let enumerator = fileMgr.enumerator(
            ...
            throw MCZFormatError.readError("无法枚举目录: \(directoryURL.path)")
        }
