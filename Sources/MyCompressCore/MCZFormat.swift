// MCZFormat.swift
// MyCompressCore - Custom .mcz archive format definitions
// Target: macOS 13+
...
/// MCZ 文件魔数: "MCZ1" (ASCII: 0x4D 0x43 0x5A 0x31)
public let MCZMagicNumber: UInt32 = 0x315A434D  // Little-endian: "MCZ1"

/// MCZ 格式版本
public let MCZFormatVersion: UInt16 = 1

// MARK: - Flags

/// 文件头标志位
public struct MCZFlags: OptionSet, Sendable {
    ...
    /// 启用 AES-256-GCM 加密
    public static let encrypted = MCZFlags(rawValue: 1 << 0)
    /// 使用固实压缩 (solid archive)
    public static let solid = MCZFlags(rawValue: 1 << 1)
}

// MARK: - Compression Algorithm

/// 支持的压缩算法
public enum MCZCompressionAlgorithm: UInt8, Sendable, CaseIterable {
    case lz4 = 0
    case zstd = 1
    case lzma = 2
...
/// - reserved2: 30 bytes
public struct MCZFileHeader: Sendable {
    public static let size: Int = 64

    public var magic: UInt32
    public var version: UInt16
    public var flags: MCZFlags
    public var algorithm: MCZCompressionAlgorithm
    ...
    public init(
        flags: MCZFlags = [],
        algorithm: MCZCompressionAlgorithm = .zstd,
        blockSize: UInt32 = 1 << 20,  // 默认 1MB
        fileCount: UInt32 = 0,
        tableOffset: UInt64 = 0,
        dataOffset: UInt64 = 0
    ) {
        self.magic = MCZMagicNumber
        self.version = MCZFormatVersion
        self.flags = flags
...
/// MCZ 文件条目元数据
public struct MCZFileEntry: Sendable {
    /// 相对路径 (以 UTF-8 存储)
...
/// MCZ 压缩块
public struct MCZBlock: Sendable {
    /// 块索引
...
public extension MCZFileHeader {
    /// 序列化为二进制数据
    func serialize() -> Data {
        var data = Data(count: MCZFileHeader.size)
        var offset = 0
    ...
    /// 从二进制数据反序列化
    static func deserialize(from data: Data) throws -> MCZFileHeader {
        guard data.count >= MCZFileHeader.size else {
            throw MCZFormatError.invalidHeaderSize(expected: MCZFileHeader.size, actual: data.count)
        }
        ...
        guard magic == MCZMagicNumber else {
            throw MCZFormatError.invalidMagicNumber(magic)
        }

        guard version <= MCZFormatVersion else {
            throw MCZFormatError.unsupportedVersion(version)
        }

        guard let algorithm = MCZCompressionAlgorithm(rawValue: algoRaw) else {
            throw MCZFormatError.unsupportedAlgorithm(algoRaw)
        }
        ...
        return MCZFileHeader(
            flags: MCZFlags(rawValue: flagsRaw),
            algorithm: algorithm,
            blockSize: blockSize,
            fileCount: fileCount,
...
public extension MCZFileEntry {
    ...
    static func deserialize(from data: Data) throws -> (entry: MCZFileEntry, bytesConsumed: Int) {
        var offset = 0
        ...
        guard offset + Int(pathLength) <= data.count else {
            throw MCZFormatError.truncatedFileTable
        }
        ...
        guard let path = String(data: pathData, encoding: .utf8) else {
            throw MCZFormatError.invalidPathEncoding
        }
...
/// 序列化整个文件表
public func serializeFileTable(_ entries: [MCZFileEntry]) -> Data {
    var table = Data()
...
/// 反序列化整个文件表
public func deserializeFileTable(from data: Data, count: Int) throws -> [MCZFileEntry] {
    var entries = [MCZFileEntry]()
    entries.reserveCapacity(count)
    var offset = 0
    let total = data.count
    for _ in 0..<count {
        let subdata = data.subdata(in: offset..<total)
        let (entry, consumed) = try MCZFileEntry.deserialize(from: subdata)
...
/// MCZ 格式相关错误
public enum MCZFormatError: Error, Sendable, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .invalidHeaderSize(let expected, let actual):
            return "MCZ 文件头大小无效: 期望 \(expected) 字节, 实际 \(actual) 字节"
