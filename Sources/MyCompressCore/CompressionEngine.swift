...
/// 压缩级别
public enum MCZCompressionLevel: Int, Sendable, CaseIterable {
    case fastest = 0
    case fast = 2
    case `default` = 5
    case best = 9
...
public struct CompressionEngine: Sendable {
    /// 压缩算法
    public let algorithm: MCZCompressionAlgorithm

    /// 压缩级别
    public let level: MCZCompressionLevel

    /// 初始化压缩引擎
    /// - Parameters:
    ///   - algorithm: 压缩算法
    ///   - level: 压缩级别, 默认 .default
    public init(algorithm: MCZCompressionAlgorithm, level: MCZCompressionLevel = .default) {
