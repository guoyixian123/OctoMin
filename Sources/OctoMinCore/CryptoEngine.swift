...
public struct OMZEncryptedData: Sendable {
    ...
    /// 从二进制数据反序列化
    public static func deserialize(from data: Data) throws -> OMZEncryptedData {
        ...
        return OMZEncryptedData(salt: salt, nonce: nonce, ciphertext: ciphertext)
    }
}
...
public actor CryptoEngine {
    ...
    public func encrypt(_ data: Data) throws -> OMZEncryptedData {
        let salt = CryptoEngine.generateSalt()
        return try encrypt(data, salt: salt)
    }

    /// 使用指定盐值加密数据 (用于确定性加密场景)
    public func encrypt(_ data: Data, salt: Data) throws -> OMZEncryptedData {
        ...
        return OMZEncryptedData(salt: salt, nonce: nonceData, ciphertext: ciphertext)
    }
    ...
    /// - Returns: 解密后的明文数据
    public func decrypt(_ encrypted: OMZEncryptedData) throws -> Data {
        ...
        let tagStart = encrypted.ciphertext.count - OMZEncryptedData.tagSize
        guard tagStart >= 0, encrypted.ciphertext.count >= OMZEncryptedData.tagSize else {
            throw CryptoError.invalidEncryptedData
        }

        let ciphertext = encrypted.ciphertext.prefix(tagStart)
        let tag = encrypted.ciphertext.suffix(OMZEncryptedData.tagSize)
    ...
    public func decryptFromData(_ data: Data) throws -> Data {
        let encrypted = try OMZEncryptedData.deserialize(from: data)
    ...
    public func decryptBlock(_ ciphertext: Data, salt: Data, nonce: Data) throws -> Data {
        ...
        let tagStart = ciphertext.count - OMZEncryptedData.tagSize
        guard tagStart >= 0, ciphertext.count >= OMZEncryptedData.tagSize else {
            throw CryptoError.invalidEncryptedData
        }
        let ct = ciphertext.prefix(tagStart)
        let tag = ciphertext.suffix(OMZEncryptedData.tagSize)
        do {
...
private let saltSize = OMZEncryptedData.saltSize
private let nonceSize = OMZEncryptedData.nonceSize
