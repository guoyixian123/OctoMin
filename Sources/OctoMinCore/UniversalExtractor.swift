...
public final class UniversalExtractor: @unchecked Sendable {
    ...
    public func extract(archive url: URL, to destination: URL, password: String? = nil, overwrite: Bool = true) async throws {
        ...
        switch format {
        case .omz:
            try await extractOMZ(url: url, to: destination, password: password, overwrite: overwrite)
        case .zip:
    ...
    // MARK: - OMZ 解压（我们自己的格式）
    private func extractOMZ(url: URL, to destination: URL, password: String?, overwrite: Bool) async throws {
        let options = OMZDecompressionOptions(
            password: password,
