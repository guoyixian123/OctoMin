...
/// 解压进度报告
public struct OMZDecompressionProgress: Sendable {
    /// 已处理文件数
...
/// 解压选项
public struct OMZDecompressionOptions: Sendable {
    /// 密码 (用于加密档案)
...
/// OMZ 档案元信息 (可用于列出内容)
public struct OMZArchiveInfo: Sendable {
    public let header: OMZFileHeader
    public let entries: [OMZFileEntry]
    /// 总未压缩大小
...
public actor ParallelDecompressor {
    /// 解压选项
    private let options: OMZDecompressionOptions

    /// 进度回调
    public typealias ProgressHandler = @Sendable (OMZDecompressionProgress) -> Void
    ...
    /// 初始化并行解压器
    public init(
        options: OMZDecompressionOptions = OMZDecompressionOptions(),
    ...
    /// 读取 OMZ 档案信息 (不解压)
    public static func readArchiveInfo(archiveURL: URL) throws -> OMZArchiveInfo {
        let handle = try FileHandle(forReadingFrom: archiveURL)
        defer { try? handle.close() }

        // 读取文件头
        let headerData = try readExact(handle: handle, count: OMZFileHeader.size, from: 0)
        let header = try OMZFileHeader.deserialize(from: headerData)
    ...
    public func extract(archive archiveURL: URL, to destinationURL: URL) async throws {
        ...
        // 1. 读取文件头
        let headerData = try Self.readExact(handle: handle, count: OMZFileHeader.size, from: 0)
        let header = try OMZFileHeader.deserialize(from: headerData)

        guard header.version == OMZFormatVersion else {
            throw OMZFormatError.unsupportedVersion(header.version)
        }
        ...
        if header.isEncrypted {
            ...
            let saltOffset = UInt64(OMZFileHeader.size)
            masterSalt = try Self.readExact(handle: handle, count: OMZEncryptedData.saltSize, from: saltOffset)
        ...
        if let crypto = crypto, let salt = masterSalt {
            ...
            let nonceData = tableRawData.prefix(OMZEncryptedData.nonceSize)
            let encTableData = tableRawData.subdata(in: OMZEncryptedData.nonceSize..<tableRawData.count)
        ...
        try await withThrowingTaskGroup(of: Int64.self) { group in
            ...
            func submitNext() {
                ...
                group.addTask { [options] in
                    ...
                    // 如果加密, 解密
                    if let crypto = crypto, let salt = masterSalt {
                        let nonceData = compressedData.prefix(OMZEncryptedData.nonceSize)
                        let encData = compressedData.subdata(in: OMZEncryptedData.nonceSize..<compressedData.count)
                    ...
                    if options.verifyCRC {
                        let actualCRC = OMZCRC32.compute(decompressed)
                        guard actualCRC == entry.crc32 else {
                            throw OMZFormatError.crcMismatch(expected: entry.crc32, actual: actualCRC)
                        }
                    }
                    ...
                    if fileMgr.fileExists(atPath: outURL.path) {
                        if options.overwriteExisting {
                            ...
                            throw OMZFormatError.writeError("文件已存在且未启用覆盖: \(entry.path)")
                        }
                    }
        ...
        // 报告完成
        progressHandler?(OMZDecompressionProgress(
    ...
    /// 列出加密档案内容 (需要密码)
    public func listContents(archive archiveURL: URL) async throws -> [OMZFileEntry] {
        let handle = try FileHandle(forReadingFrom: archiveURL)
        defer { try? handle.close() }
        ...
        if header.isEncrypted {
            ...
            crypto = CryptoEngine(password: password)
            masterSalt = try Self.readExact(handle: handle, count: OMZEncryptedData.saltSize, from: UInt64(OMZFileHeader.size))
        ...
        if let crypto = crypto, let salt = masterSalt {
            let nonceData = tableRawData.prefix(OMZEncryptedData.nonceSize)
            let encData = tableRawData.subdata(in: OMZEncryptedData.nonceSize..<tableRawData.count)
    ...
    private static func readExact(handle: FileHandle, count: Int, from offset: UInt64) throws -> Data {
        ...
        guard data.count == count else {
            throw OMZFormatError.readError("无法读取足够数据: 期望 \(count) 字节, 实际 \(data.count) 字节")
        }
        return data
    }
    ...
    private func reportProgress(totalFiles: Int, totalBytes: Int64, currentFile: String?) {
        let progress = OMZDecompressionProgress(
