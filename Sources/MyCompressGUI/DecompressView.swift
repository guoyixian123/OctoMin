...
struct DecompressView: View {
    ...
    @State private var mczInfo: MCZArchiveInfo?
        ...
        ZStack {
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    // 密码选项（仅加密档案显示）
                    if archiveURL != nil && (archiveInfo?.isEncrypted == true || mczInfo?.header.isEncrypted == true) {
                        passwordSection
                            .padding(.horizontal, 20)
                            .padding(.top, 12)
                    }
                    ...
                    if archiveURL != nil {
                        overwriteSection
                            .padding(.horizontal, 20)
                            .padding(.top, archiveInfo?.isEncrypted == true || mczInfo?.header.isEncrypted == true ? 0 : 12)
                    }
    ...
    private var dropZone: some View {
        Button(action: selectArchive) {
            VStack(spacing: 8) {
                ...
                Text("支持 .mcz .zip .rar .7z .tar .gz .bz2 等")
                    .font(.system(size: 10))
    ...
    private func archiveInfoView(url: URL) -> some View {
        ...
        HStack(spacing: 12) {
            ...
            Button(action: { archiveURL = nil; archiveInfo = nil; mczInfo = nil }) {
    ...
    private func selectArchive() {
        ...
        panel.message = "选择压缩文件（支持 .mcz .zip .rar .7z .tar .gz 等）"
    ...
    private func loadArchiveInfo() {
        ...
        Task {
            do {
                if let format = ArchiveFormat.from(url: url), format == .mcz {
                    mczInfo = try ParallelDecompressor.readArchiveInfo(archiveURL: url)
                }
